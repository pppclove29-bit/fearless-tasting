...
**🟢 업로드를 막는 것 — 없다 (2026-09-23 확인)**

목적은 **출시 절차를 실제로 한 번 통과시켜 보는 것**이다. 완성도로 진행을 막지 않는다.
진행을 막는 기준은 **Play Console이 실제로 거부하는가** 하나다 (아래 표).

그 기준으로 보면 **지금 내부 테스트 업로드를 막는 항목은 하나도 없다.**
`✅ 막음` 4종(서명 AAB · 512 아이콘 · 앱 이름/설명 · 개발자 계정)이 전부 준비됐다.

- [ ] ⚠️ **업로드 전 유일한 사람 작업: keystore 백업** — `~/keys/` 폴더
      (키 3개 + 비밀번호 3개, §5). 업로드를 막지는 않지만, **이 키로 한 번 올린 뒤
      키를 잃으면 같은 앱을 영구히 업데이트 못 한다.** 되돌릴 방법이 없으므로
      업로드보다 먼저 해둘 것

심사용 카카오 계정은 **업로드 요건이 아니라 프로덕션 심사 요건**이다(표 참고).
내부 테스트는 심사가 사실상 없으므로 계정을 기다리지 말고 먼저 올린다.
계정이 생기면 아래 셋이 한꺼번에 풀린다 — 로그인 왕복 검증 · 방 목록/방 생성 검증 ·
로그인 후 화면 스크린샷 (§10, 프로덕션 전까지 하면 된다).

**✅ 업로드에 필요한 것 중 이미 된 것**

- [x] 릴리스 keystore + `keystore.properties` (§5) — 2026-08-18, 2026-09-21 `~/keys/`로 이동
- [x] 서명된 릴리스 AAB 빌드 검증 — 2026-09-21 키 이동 후 재검증 (4.8MB, §5)
- [x] Play Console 개발자 계정 등록 ($25) — 2026-09-21, **계정 유형 개인**
- [x] API CORS 허용 목록 수정 + 배포·검증 (§9) — 2026-09-21. 기존 웹 무영향 확인
- [x] 배포 후 앱 실검증 (§8.1) — **CORS 에러 0건**, 오프라인·뒤로가기·OAuth 브라우저 기동 OK.
      로그인 자격 증명 입력 구간만 계정 대기
- [x] **스토어 자산 전부** (§7) — 512 아이콘 · 1024×500 그래픽 · 스크린샷 2장.
      `docs/store-assets/` 에 있고 스크립트로 재생성된다
- [x] 앱 이름·설명 초안 — [store-listing.md](store-listing.md)

### 무엇이 업로드를 실제로 막는가

우선순위를 정할 때 쓰는 기준. **세 앱에 공통으로 적용된다.**
"있으면 좋은 것"과 "없으면 콘솔이 거부하는 것"을 섞지 않는다.

| 항목 | 업로드를 막나 | 비고 |
| --- | :---: | --- |
| 서명된 AAB | ✅ 막음 | 없으면 시작도 못 한다 |
| 512×512 아이콘 | ✅ 막음 | 등록정보 필수 입력 |
| 1024×500 그래픽 | ⚠️ 아마 | 내부 테스트에선 보통 불필요. 확신 못 해 미리 만들어 뒀다 |
| 스크린샷 2장 | ⚠️ 아마 | 위와 같음 |
| 앱 이름·설명 | ✅ 막음 | 등록정보 필수 입력 |
| 심사용 테스트 계정 | ❌ 안 막음 | **프로덕션 심사** 요건. 내부 테스트는 심사가 사실상 없다 |
| 로그인 왕복 검증 | ❌ 안 막음 | 품질 문제지 업로드 요건이 아니다 |
| keystore 백업 | ❌ 안 막음 | 다만 분실하면 **올린 앱을 영구히 업데이트 못 한다** |
| 데이터 안전·콘텐츠 등급 | ❌ 안 막음 | 프로덕션 출시 전까지만 하면 된다 |
| `assetlinks.json` | ❌ 안 막음 | 첫 업로드 후에야 지문을 받는다. 없으면 링크가 브라우저로 열릴 뿐 |

⚠️ 표시는 **확신할 수 없는 항목**이다. 만드는 비용이 낮으면 미리 만들어
불확실성을 없애고, 비싸면 콘솔이 막을 때까지 미룬다.

**🟡 클로즈드 테스트 도중에 해도 되는 것**

테스터가 14일을 채우는 동안 병렬로 진행하면 된다.

- [ ] **심사용 테스트 카카오 계정 + 샘플 데이터 (§10)** — 2단계 인증 해제 필수.
      **프로덕션 액세스 신청 전까지** 준비하면 된다. 내부 테스트 업로드는 막지 않는다
...
- [ ] 스토어 설명 문구 다듬기
- [ ] **스크린샷 교체** — 지금은 로그인 전 랜딩 화면 2장이다. 계정이 생기면
      방 목록·통계 같은 실제 사용 화면으로 바꾸는 게 좋다 (§7에 방법)
- [ ] versionCode 관리 — 첫 업로드는 `1`로 OK, **2번째 업로드부터 증가 필수** (§6)
...
| 2026-09-21 | 운영 API (배포 후) | CORS 수정 배포·검증 통과. 기존 웹 무영향 (§9) |
| 2026-09-21 | 에뮬레이터 (배포 후 재검증) | **CORS 에러 0건.** 오프라인·뒤로가기·OAuth 브라우저 기동 OK. 로그인 자격 증명 구간만 미확인 (§8.1) |
...
세 앱의 키와 비밀번호가 전부 `~/keys/`에 모여 있다 (전부 권한 600).
**이 폴더 하나만 백업하면 세 앱이 모두 커버된다.**

```
~/keys/
├── fearless-upload.jks      + fearless.pass      ← 이 앱
├── typeright-upload.jks     + typeright.pass
└── driverlabor-upload.jks   + driverlabor.pass
```

- [ ] `~/keys/` 폴더를 안전한 곳에 백업 (이 앱 기준 아직 미확인)

빌드는 여전히 `apps/web/android/keystore.properties`에서 비밀번호를 읽는다.
`~/keys/fearless.pass`는 **백업용 사본**이고, 둘의 값이 같아야 한다.
비밀번호를 바꾸면 두 곳을 같이 고칠 것.

> ⚠️ 키와 비밀번호를 분실하면 **같은 앱으로 영구히 업데이트 불가**. 새 패키지명으로
> 새 앱을 내야 하고 기존 설치자는 옮겨오지 못한다.
> 로컬 디스크 한 곳에만 있으면 백업이 아니다 — 물리적으로 다른 매체에 둘 것.
...
자산은 전부 `docs/store-assets/` 에 있고, 스크립트로 재생성한다.
의존성 없이 표준 라이브러리만 쓴다 (이 머신에 PIL·ImageMagick 이 없다).

- [ ] 앱 이름 / 짧은 설명(80자) / 자세한 설명(4000자) — 초안: [store-listing.md](store-listing.md)
- [x] **512×512 앱 아이콘** — `docs/store-assets/icon-512.png`
- [x] **1024×500 그래픽 이미지** — `docs/store-assets/feature-graphic-1024x500.png`
- [x] **스크린샷 2장** — `docs/store-assets/screenshots/screenshot-{1,2}.png`

| 파일 | 규격 | 생성 |
| --- | --- | --- |
| `icon-512.png` | 512×512, 32비트, 투명 0 | `python3 docs/store-assets/make-icon.py` |
| `feature-graphic-1024x500.png` | 1024×500, 32비트, 투명 0 | `python3 docs/store-assets/make-banner.py` |
| `screenshots/screenshot-*.png` | 1080×1920 (9:16) | `python3 docs/store-assets/make-screenshots.py <캡처...>` |

세 스크립트 모두 **생성 후 자체 검증**한다 (크기·투명 픽셀). 조건을 못 맞추면
조용히 잘못된 파일을 내놓지 않고 실패한다.

#### 아이콘

앱에 들어 있는 커스텀 아이콘("무식가")을 내보낸 것이다. 기본 Capacitor 아이콘이 아니다.

원본 `apps/web/public/icons/icon-512.png`(PWA 아이콘)는 그대로 쓸 수 없다 —
**모서리가 둥글고 그 바깥이 투명**하기 때문이다. Play 는 투명도를 허용하지 않고
둥근 모서리·그림자도 직접 넣지 말라고 한다(스토어가 알아서 마스킹한다).
스크립트가 아이콘 본체와 **같은 색(`#111111`)으로 알파를 합성**해 모서리를 메운다.

> 알파 채널은 남기되 값을 전부 `255` 로 채웠다.
> "32비트 PNG" 요구와 "투명 불가" 요구를 동시에 만족시키기 위해서다.

#### 그래픽 이미지 (배너)

앱 안에 배너용 소스가 없어서 512 아이콘을 같은 색 캔버스 중앙에 올렸다.
아이콘에 워드마크가 이미 있어 텍스트 렌더링 없이 배너로 성립하고,
배경색이 같아 이어 붙인 티가 나지 않는다.

#### 스크린샷 — 계정이 생기면 교체할 것

에뮬레이터는 1080×2400(9:20)이라 Play 의 9:16 한계보다 길쭉하다.
그래서 위에서부터 1080×1920 으로 자른다(헤더와 주요 콘텐츠가 위에 있다).

지금 것은 **로그인 전 랜딩 화면 2장**이다. 심사용 계정(§10)이 생기면
방 목록·식당 목록·통계 같은 **실제 사용 화면으로 교체**하는 게 좋다.

```bash
ADB=~/Library/Android/sdk/platform-tools/adb
$ADB -s emulator-5554 exec-out screencap -p > /tmp/shot1.png
$ADB -s emulator-5554 exec-out screencap -p > /tmp/shot2.png
python3 docs/store-assets/make-screenshots.py /tmp/shot1.png /tmp/shot2.png
```

> ⚠️ 알려진 흠: `screenshot-1.png` 은 오프라인 테스트 중 찍어서
> **상태바에 네트워크 오류 아이콘**이 있다. 업로드를 막지는 않지만
> 교체할 때 같이 고치면 된다.
...
마지막 검증: **2026-09-21** (CORS 배포 후, 에뮬 `fearless_test` · Pixel 6 · Android 15 · arm64)

| 항목 | 결과 |
| --- | --- |
| 앱 설치 → 기동 → 홈 렌더링 | ✅ OK |
| **API 호출 (CORS)** | ✅ OK — 콘솔에 CORS·네트워크 에러 0건. 배포 전 전 기능을 막던 문제 해소 확인 |
| 하드웨어 뒤로가기: 루트에서 뒤로 → 종료 | ✅ OK (`canGoBack:false` → 종료) |
| 카카오 로그인 **① 시스템 브라우저 열림** | ✅ OK — Capacitor Browser 플러그인이 Chrome 커스텀 탭으로 `accounts.kakao.com` 로드. 앱 내 웹뷰 아님 |
| 카카오 로그인 **② 자격 증명 입력** | ⏸️ **미확인 — 심사용 카카오 계정 필요** (§10). 사람이 눌러야 하는 구간 |
| 카카오 로그인 **③ 딥링크 복귀 → 토큰 저장** | ✅ OK — `kr.fearlesstasting.app://login?access_token=...` 주입 시 `/login?access_token=...` 진입 확인 (adb 주입) |
| 방 목록 진입 / 방 생성 | ⏸️ **미확인 — 로그인 필요.** ②가 풀려야 확인 가능 |
| 오프라인(네트워크 차단) 실행 | ✅ OK — 껍데기 UI 정상 렌더링, 백지·크래시 없음. 네트워크 복구 후 에러 없이 재기동 |
| 푸시 권한 요청 + FCM 토큰 등록 | ⏸️ 미확인 (에뮬에 Play services 있음 → 확인 가능) |
| 공개 방/커뮤니티 링크 → 시스템 브라우저 | ⏸️ 미확인 |

> **로그인 왕복은 양 끝이 검증됐고 가운데만 남았다.**
> 나가는 길(①)과 돌아오는 길(③)은 확인했다. 실제 자격 증명 입력(②)은
> 카카오 계정이 있어야 하므로 **심사용 계정(§10)을 만들 때 함께 확인한다.**
> 그 계정이 생기면 방 목록·방 생성도 같이 풀린다.
