...
- [~] 배포 후 앱 실검증 (§8.1) — CORS 해소·오프라인·뒤로가기·브라우저 기동 확인.
      **로그인 자격 증명 입력과 방 생성은 심사용 계정이 있어야 한다** (아래 항목과 묶인다)
- [ ] 심사용 테스트 카카오 계정 + 샘플 데이터 (§10) — 심사자가 로그인 못 하면 거부된다
- [~] 스토어 등록정보 (§7) — 512 아이콘 완료, 설명 초안 있음.
      **스크린샷은 샘플 데이터 이후, 1024×500 그래픽은 사람이 디자인해야 한다**
...
- [ ] 스크린샷 폰 최소 2장 (16:9 또는 9:16, 최소 320px)
      ⚠️ 샘플 데이터가 있어야 볼 만한 화면이 나온다. 심사용 계정(§10) 이후에 찍을 것
- [x] **512×512 앱 아이콘** — `docs/store-assets/icon-512.png` (2026-09-21 생성)
- [ ] 1024×500 그래픽 이미지 — **사람이 디자인해야 한다** (앱 안에 소스가 없다)
