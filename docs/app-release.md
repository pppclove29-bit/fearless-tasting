...
- [x] 512×512 앱 아이콘 (§7) — `docs/store-assets/icon-512.png`
- [x] 앱 이름·설명 초안 — [store-listing.md](store-listing.md)

**🟡 콘솔이 요구하면 그때 하는 것**

내부 테스트 트랙에서는 보통 요구되지 않는다(스토어 등록정보 완성은 프로덕션 출시 요건).
**확신할 수 없으니 콘솔에서 막히면 그때 만든다.**

- [ ] 1024×500 그래픽 이미지 — 앱 안에 소스가 없다. 막히면 아이콘 기반으로 배너 생성 가능
- [ ] 스크린샷 폰 2장 — 랜딩 화면은 지금도 찍을 수 있고, 로그인 후 화면은 계정 이후
...
- [x] **512×512 앱 아이콘** — `docs/store-assets/icon-512.png` (2026-09-21 생성)
- [ ] 스크린샷 폰 최소 2장 (16:9 또는 9:16, 최소 320px)
- [ ] 1024×500 그래픽 이미지

> **아래 둘은 내부 테스트 단계에서는 보통 요구되지 않는다.**
> 스토어 등록정보 완성은 프로덕션 출시 쪽 요건이다. 확신할 수 없는 항목이니
> **콘솔이 실제로 막으면 그때 만든다.**
>
> - 스크린샷: 랜딩 화면은 에뮬레이터로 지금도 찍을 수 있다.
>   로그인 후 화면은 심사용 계정(§10) 이후.
> - 1024×500 그래픽: 앱 안에 소스가 없다. 필요해지면 512 아이콘을
>   검정(`#111111`) 캔버스 중앙에 배치한 배너로 만들 수 있다
>   (아이콘에 "무식가" 워드마크가 이미 있어 배너로 성립한다).

#### 512 아이콘 생성 방법

기존 앱 아이콘에서 내보낸 것이고, 새로 디자인한 게 아니다.
앱에 들어 있는 아이콘은 **기본 Capacitor 아이콘이 아니라 커스텀 아이콘**("무식가")이라
그대로 쓸 수 있다.
...
원본은 `apps/web/public/icons/icon-512.png`(PWA 아이콘)인데 그대로는 못 쓴다 —
**모서리가 둥글고 그 바깥이 투명**하기 때문이다. Play 는 투명도를 허용하지 않고,
둥근 모서리·그림자도 직접 넣지 말라고 한다(스토어가 알아서 마스킹한다).

그래서 스크립트가 아이콘 본체와 **같은 색(`#111111`)으로 알파를 합성**해
모서리를 메운다. 색이 같으므로 이어 붙인 티가 안 나고 결과는 꽉 찬 정사각형이 된다.

| 항목 | 값 |
| --- | --- |
| 크기 | 512×512 |
| 형식 | 32비트 PNG (알파 채널 있음, 전 픽셀 `255`) |
| 투명 픽셀 | **0개** (스크립트가 생성 후 자체 검증하고, 남으면 실패한다) |
| 둥근 모서리 | 없음 (꽉 찬 사각형) |

> 아이콘 디자인이 바뀌면 `apps/web/public/icons/icon-512.png`를 갱신한 뒤
> 위 스크립트를 다시 돌린다. 의존성 없이 표준 라이브러리만 쓴다.
