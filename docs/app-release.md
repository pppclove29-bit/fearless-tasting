...
**🔴 지금 막고 있는 것 — 심사용 카카오 계정 하나**

목적은 **출시 절차를 실제로 한 번 통과시켜 보는 것**이다. 완성도로 진행을 막지 않는다.
진행을 막는 기준은 **Play Console이 실제로 거부하는가** 하나다.

- [ ] **심사용 테스트 카카오 계정 + 샘플 데이터 (§10)** ← 유일한 🔴
      2단계 인증 해제 필수. 이 계정이 생기면 아래 셋이 한꺼번에 풀린다:
      로그인 왕복 검증 · 방 목록/방 생성 검증 · 로그인 후 화면 스크린샷
- [ ] ⚠️ **keystore 백업 확인** — `~/keys/` 폴더 (키 3개 + 비밀번호 3개, §5).
      업로드를 막지는 않지만 분실하면 **올린 앱을 영구히 업데이트 못 한다.**
      절차 통과가 목적이라면 더더욱 먼저 해둘 것
